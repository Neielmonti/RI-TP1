...
class EJ3:
    def __init__(self, corpus_path: Path, queries_path: Path):
        files_n = len([f for f in os.listdir(corpus_path) if os.path.isfile(os.path.join(corpus_path, f))])
...
def main():
    ...
    parser.add_argument("corpus_path", type=str, help="Ruta al directorio con archivos HTML")
